...
arbitraryExAv pn v db =
    (\comps -> ExAv (unPN pn) (unPV v) comps []) <$> arbitraryExampleComponents pn db
...
arbitraryExampleComponents :: PN -> TestDb -> Gen (ExampleComponents [ExampleDependency])
arbitraryExampleComponents _  (TestDb []) =
    return defaultExampleComponents { exLib = Just [] }
arbitraryExampleComponents pn db          = do
  -- dedupComponentNames removes components with duplicate names, for example,
  -- 'ComponentExe x' and 'ComponentTest x', and then toExampleComponents
  -- deduplicates unnamed components.
  comps <- toExampleComponents . dedupComponentNames . filter (isValid . fst)
             <$> boundedListOf 5 (arbitraryComponent db)
  return $ if isCompleteExampleComponents comps
           then comps
           else -- Add a library if the ExampleComponents isn't complete.
                comps { exLib = Just [] }
  where
    ...
    toExampleComponents comps = ExampleComponents {
      ...
      , exBenchmarks = [(name, deps) | (ComponentBench name, deps) <- comps]
      , exSetup = listToMaybe [deps | (ComponentSetup, deps) <- comps]
      }
...
-- | Returns true if the ExampleComponents forms a complete package, i.e., it
-- contains at least one library, exe, test, or benchmark.
isCompleteExampleComponents :: ExampleComponents a -> Bool
isCompleteExampleComponents comps =
     isJust (exLib comps)
  || not (null (exExes comps))
  || not (null (exTests comps))
  || not (null (exBenchmarks comps))

arbitraryComponent :: TestDb -> Gen (Component, [ExampleDependency])
arbitraryComponent db = do
  comp <- arbitrary
  deps <- case comp of
...
instance (Arbitrary a, Monoid a) => Arbitrary (ExampleComponents a) where
  arbitrary = error "arbitrary not implemented: ExampleComponents"

  shrink comps =
      filter isCompleteExampleComponents $
      [ ExampleComponents {
            exLib = mLib
          , exSubLibs = subLibs
          , exFLibs = fLibs
          , exExes = exes
          , exTests = tests'
          , exBenchmarks = benchmarks
          , exSetup = setup
          }
      | (mLib, subLibs, fLibs, exes, tests', benchmarks, setup)
         <- shrink ( exLib comps
                   , exSubLibs comps
                   , exFLibs comps
                   , exExes comps
